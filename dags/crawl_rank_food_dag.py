...
from crawler.crawl_rank_food import get_top100, get_product_detail_info
from airflow.providers.amazon.aws.hooks.s3 import S3Hook
# import boto3
import logging

def crawl_food_data(**context):
    logging.info("get_top100_food 실행")
    data, goods_no_list = get_top100()
    ...
    output_path = f"/opt/airflow/data/food_{ts}.json"
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    result_df.to_json(output_path, orient="records", force_ascii=False, indent=2)

    context['ti'].xcom_push(key='food_file_path', value=output_path)
    logging.info(f"JSON 저장 완료!: {output_path}")

def upload_to_s3(**context):
    file_path = context['ti'].xcom_pull(key='food_file_path', task_ids='crawl_food')
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    s3_key = f"raw_data/non_pb/food/{ts}.json"
...
with DAG(
    ...
    crawl_food = PythonOperator(
        task_id="crawl_food",
        python_callable=crawl_food_data,
