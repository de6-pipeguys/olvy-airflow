...
from crawler import crawl_rank_food
...
def crawl_top100_food(**context):
    chromedriver_autoinstaller.install()
    data, goods_no_list = crawl_rank_food.get_top100_food()
...
def crawl_product_info(**context):
    ...
    with SB(uc=True, test=True, headless=True) as sb:
        ...
        for idx, goods_no in enumerate(goods_no_list, 1):
            detail = crawl_rank_food.get_product_detail_info(sb, goods_no)
    ...
    #filename = f"rank_food_result_{timestamp}_%H%M%S.json"
    if dag_id == 'rank_crawl_afternoon_food':
        time_str = "170100"
    elif dag_id == 'rank_crawl_morning_food':
        time_str = "093000"
    else:
        time_str = "000000"

    filename = f"rank_food_result_{timestamp}_{time_str}.json"
...
with DAG(
    dag_id='rank_crawl_morning_food',
    ...
    crawl_top100 = PythonOperator(
        ...
        python_callable=crawl_top100_food,
...
with DAG(
    dag_id='rank_crawl_afternoon_food',
    default_args=default_args,
