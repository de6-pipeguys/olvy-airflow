...
def crawl_cleansing_data(**context):
    ...
    if dag_id == "crawl_cleansing_afternoon_dag":
        time_str = "170100"
    elif dag_id == "crawl_cleansing_morning_dag":
        time_str = "093000"
    else:
        time_str = "000000"

    ts = datetime.now().strftime("%Y%m%d")
    filename = f"rank_cleansing_result_{ts}_{time_str}.json"
...
with DAG(
    dag_id="crawl_cleansing_afternoon_dag",
    default_args=default_args,
