...
from crawler import crawl_rank_manscare
...
def crawl_top100_manscare(**context):
    chromedriver_autoinstaller.install()
    data, goods_no_list = crawl_rank_manscare.get_top100_manscare()
...
def crawl_product_info(**context):
    ...
    with SB(uc=True, test=True, headless=True) as sb:
        ...
        for idx, goods_no in enumerate(goods_no_list, 1):
            detail = crawl_rank_manscare.get_product_detail_info(sb, goods_no)
    ...
    #filename = f"rank_manscare_result_{timestamp}_%H%M%S.json"
    if dag_id == 'rank_crawl_afternoon_manscare':
        time_str = "170100"
    elif dag_id == 'rank_crawl_morning_manscare':
        time_str = "093000"
    else:
        time_str = "000000"

    filename = f"rank_manscare_result_{timestamp}_{time_str}.json"
...
def upload_to_s3(**context):
    ...
    s3_object_name = f"raw_data/non_pb/manscare/{filename}"
...
with DAG(
    dag_id='rank_crawl_afternoon_manscare',
    default_args=default_args,
    description='manscare ranking crawl(afternoon)',
    #schedule_interval="5 16 * * *",  # airflow 2 버전
    schedule="1 17 * * *",        # airflow 3 버전
    start_date=datetime(2024, 7, 1, tzinfo=timezone("Asia/Seoul")),
    catchup=False,
    tags=['manscare', 'ranking', 'afternoon'],
) as dag:
