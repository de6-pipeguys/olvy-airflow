...
import os
from crawler import crawl_brand_food
...
def crawl_pb_brand(**context):
    ...
    for brand_name, brand_code in PB_BRAND_CODE_DICT.items():
        data, goods_no_list = crawl_brand_food.get_brand(brand_name, brand_code)
    ...
    for brand_name, brand_info in brand_data.items():
        ...
        with SB(uc=True, test=True, headless=True) as sb:
            for idx, goods_no in enumerate(goods_no_list, 1):
                detail = crawl_brand_food.get_brand_product_detail_info(sb, goods_no)
        ...
        # 파일명 생성
        brand_file_key = brand_name.lower().replace(" ", "_").replace("딜라이트_프로젝트", "delightproject")
